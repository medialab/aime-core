    ...
    mysql.query('SELECT * FROM tbl_items', function(err, rows) {
      ...
      chapters.forEach(function(chapter, i) {
        ...
        if (chapter.content_fr &&
            ...
            chapter.content_fr !== 'plan d’ensemble') {
          fr = book.save({
            type: 'chapter',
            ...
            mysql_model: 1,
            mysql_id: chapter.id,
            displayNumber: (~chapter.content_fr.search(/(introduction|conclusion)/)) ? '·' : (++displayNumber)
          });
        ...
        if (chapter.content_en && chapter.content_en !== 'englishContent') {
          en = book.save({
            ...
            mysql_model: 1,
            mysql_id: chapter.id,
            displayNumber: (~chapter.content_en.search(/(introduction|conclusion)/)) ? '·' : displayNumber
          });
