  ...
  return function(topNext) {
    async.waterfall([
      function fetchImages(next) {
        neo4j.db.query('MATCH (n:`Media`) RETURN n;', function(err, results) {
          var imagesIndex = {};
        ...
        mysql.query('SELECT * from tbl_documentItems', function(err, rows) {
          ...
          videos.forEach(function(t) {
            var node = batch.save({
              ...
              kind: 'video',
              host: 'vimeo',
              iframe: '<iframe class="vimeo" data-src="http://player.vimeo.com/video/' + t.content_fr + '?title=0&byline=0&portrait=0&color=ffffff" frameborder="0" webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen="" src="http://player.vimeo.com/video/' + t.content_fr + '?title=0&byline=0&portrait=0&color=ffffff"></iframe>',
