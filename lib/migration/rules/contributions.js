    ...
    async.waterfall([
      function batch(next) {
        var fields = [
          ...
          '`tbl_contributionItems`.create_time AS create_time',
          '`tbl_contributionItems`.update_time AS update_time',
          ...
          function(err, rows) {
            ...
            titles.forEach(function(title) {
              var contributionNode = batch.save({
                ...
                mysql_id: title.id,
                mysql_model: 4,
                date: title.create_time.split(' ')[0].replace(/-/g, ''),
                last_update: title.update_time.split(' ')[0].replace(/-/g, '')
              });
              ...
              slides.forEach(function(slide, i) {
                ...
                slideElements.forEach(function(e, i) {
                  ...
                  if (e.type === 4) {
                    ...
                    var kind = data.type === 'photo' ? 'image' : data.type;

                    if (data.type === 'rich') {
                      if (!/iframe/.test(data.html))
                        kind = 'link';
                      else if (/scribd/.test(data.url))
                        kind = 'pdf';
                      else
                        kind = 'rich';
                    }
                    ...
                    if (kind === 'image' && /modesofexistence\.org/.test(data.url)) {
                      data.url = data.url.replace(/(http:\/\/)?(www\.)?modesofexistence\.org\/medias\//, '');

                      eNode = batch.save({
                        type: 'media',
                        internal: true,
                        kind: 'image',
                        path: 'crossings/' + data.url
                      });
                    }
                    else {
                      eNode = batch.save({
