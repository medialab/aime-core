  ...
  return function(topNext) {
    async.waterfall([
      function cypher(next) {
        ...
        neo4j.db.query('MATCH n WHERE (n:Book OR n:Paragraph OR n:Document OR n:Vocabulary OR n:Contribution) AND NOT (n)<-[:HAS]-(:Slide) RETURN n;', next);
      },
      function relations(entities, next) {
        var batch = neo4j.db.batch();

        // Fetching links in db
        mysql.query('SELECT * FROM tbl_bookmarks WHERE type != -1', function(err, links) {
