...
module.exports = function(mysql, neo4j) {
  ...
  return function(topNext) {
    async.waterfall([
      ...
      {
        neo4j.db.query('MATCH (c:Document {source_platform:"crossings"})-[rt:RELATES_TO]-(mc:Crossing)-[d:DEFINES]-(v:Vocabulary) WHERE c.lang=v.lang return c,rt,mc,d,v LIMIT 20',next);
      },
      function crossings_contrib_links(links, next) {
        ...
        links.forEach(function(link){
          ...
          batch.relate(link["v"].id, 'CITES', link["c"].id, {
              });
        });
