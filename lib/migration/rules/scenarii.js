  ...
  return function(topNext) {
    async.waterfall([
      function(next) {
        async.parallel({
          nodes: function(n) {
            neo4j.db.query('MATCH n WHERE (n:User AND n.email = "modesofexistence@gmail.com") OR n:Subheading OR n:Vocabulary OR n:Contribution OR n:Document OR n:Crossing OR n:Mode RETURN n;', n);
          },
          scenarii: function(n) {
      ...
      function(data, next) {
        var batch = neo4j.db.batch(),
            team = _.find(data.nodes, {email: 'modesofexistence@gmail.com'});

        // NOTE: Team AIME => Bruno Latour
        team.name = 'Bruno';
        team.surname = 'Latour';
        ...
        data.scenarii.forEach(function(scenario) {
          ...
          batch.relate(scenarioNode, 'CREATED_BY', team);
        });
