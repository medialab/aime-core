      ...
      modesCrossingsLinks: function(next){
        async.series({
          ...
          paragraphs:function(n){
            neo4j.db.query('MATCH (np) WHERE (np:BookItem OR np:Vocabulary) AND np.text =~ "(?ius).*\[[a-z]{2,3}(·[a-z]{2,3})?\].*" RETURN np',n);
          }
        },
        function(err,results){
          ...
          results.paragraphs.forEach(function(p) {
            ...
            p.markdown=p.markdown.replace(/([^\[])(\[{1}[a-z]{2,3}(·[a-z]{2,3})?\])([^\]])/g,
              function(m,prefix,modecrossing,dump,suffix,offset,paragraph){
                //ajouter le lien

                if(!~existing_links.indexOf("voc_"+modesCrossingsVocs[p.lang][modecrossing].slug_id))
                {
                  batch.relate(p.id, 'CITES', modesCrossingsVocs[p.lang][modecrossing].id);
                  existing_links.push("voc_"+modesCrossingsVocs[p.lang][modecrossing].slug_id)
                }

                return prefix+"["+modecrossing+"](voc_"+modesCrossingsVocs[p.lang][modecrossing].slug_id+")"+suffix;
              }
            );
            batch.save(p);
          });
