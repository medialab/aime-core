  ...
  return function(topNext) {
    async.waterfall([
      function cypher(next) {
        ...
        neo4j.db.query('MATCH n WHERE (n:Book OR n:Paragraph OR n:Document OR n:Vocabulary) AND NOT (n)<-[:HAS]-(:Slide) RETURN n;', next);
      },
        ...
        mysql.query('SELECT * FROM tbl_links', function(err, rows) {
          ...
          _(rows)
            .filter(function(row) {
              ...
              return (row.type === 0 || (row.type === 6 && row.to_model === 3)) && row.from_id !== row.to_id;
            })
          ...
          // (Bold) and Italic (no more bold due to filter)
                ...
                _(formats)
                  ...
                  .forEach(function(f) {
                    ...
                    if (!frNode.italics)
                      ...
                      frNode.italics += '|';
                    frNode.italics += f.fr.start + ',' + f.fr.stop;
                ...
                _(formats)
                  ...
                  .forEach(function(f) {
                    ...
                    if (!enNode.italics)
                      ...
                      enNode.italics += '|';
                    enNode.italics += f.en.start + ',' + f.en.stop;
