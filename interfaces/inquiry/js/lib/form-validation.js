var __formValidation = {
  ...
  validations: {
    ...
    checked: function(input, data) {
      return __formValidation.inputs[input].input.prop('checked');
    }
  },
...
jQuery.prototype.setValidationRules = function(rules, callback, options) {
  ...
  if(options.overrideRules)
    __formValidation.forms[id].data = rules;

  if(!__formValidation.forms[id].active)
    __formValidation.forms[id].form.on('submit', onSubmit);
  __formValidation.forms[id].active = true;
};