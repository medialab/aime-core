;(function() {
   ...
   *  > }
   */
  blf.modules.customInputs.KeywordField = function(obj, controller) {
    ...
    _dom = $(blf.templates.get('KeywordField')({
    ...
    function addLanguage(data) {
      ...
      if (data.values)
        $('textarea', li).val(data.values.join(', '));
    ...
    function _validate() {
      ...
      if (obj.required && (!data || !data.length)) {
        $('.message', _dom).text(i18n.t('customInputs:KeywordField.errors.at_least_one'));
        return false;
      }
    ...
    function _getData() {
      ...
      $('ul.languages-list > li', _dom).each(function() {
        var li = $(this),
            val = mlab.string.trim($('> textarea', li).val()) || '';
        ...
        // Trim keywords:
        val = val.map(mlab.string.trim).filter(mlab.fn.id);

        // Check that there are keywords:
        if (val.length)
          languages.push({
